...
import { formatAmount, getCategoryLabel, getEligibilityIcon } from '@/lib/utils';
...
export default function PolicyDetailView({
  policy,
  backHref = '/results',
  emptyDescription = '삭제됐거나 잘못된 링크예요',
    ...
    return (
      <AppShell>
        <div className="flex flex-col items-center justify-center min-h-screen px-6 text-center">
          ...
          <p className="text-sm text-[#888] mb-6">{emptyDescription}</p>
          <Link href={backHref} className="bg-[#1B6B4A] text-white font-bold py-3 px-8 rounded-xl">
            혜택 목록으로
          </Link>
        </div>
      </AppShell>
      ...
      <div className="flex flex-col">
        <header className="bg-white px-5 pt-12 pb-4 lg:pt-8 sticky top-0 z-30 border-b border-gray-50">
          <div className="flex items-center justify-between">
            <Link href={backHref} className="p-2 -ml-2 rounded-full hover:bg-gray-100" aria-label="혜택 목록으로">
          ...
          {policy.eligibility?.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {policy.eligibility.map((rule, index) => (
                  <div key={index} className="bg-white rounded-2xl p-4 border border-gray-100 flex gap-3 items-start">
