...
const POPULAR_ALL = /실업급여|고용보험.*(?:급여|지원)|전국민\s*마음투자|마음투자\s*지원|에너지\s*바우처|통신비\s*(?:감면|지원)|인플루엔자\s*국가예방접종|긴급복지\s*(?:지원|생계|의료|주거)|주거급여|기초생활\s*보장|생계급여|의료급여/;

// 청년 대상 핵심 인기 정책
const POPULAR_YOUTH = /청년도약계좌|청년내일저축계좌|청년희망적금|청년월세.*(?:지원|특별)|청년\s*구직활동\s*지원금|청년수당|청년\s*주거급여|버팀목\s*전세|청년전세임대|행복주택|청년\s*매입임대|청년\s*신혼부부\s*매입임대/;

// 취업/재직자 핵심 정책
const POPULAR_WORK = /두루누리\s*사회보험|내일채움공제|청년내일채움공제|근로장려금|자녀장려금|출산전후휴가\s*급여|육아휴직\s*급여|배우자\s*출산휴가/;

// 교육/대출 핵심 정책
const POPULAR_EDU = /국가장학금|학자금\s*대출|버팀목전세자금|디딤돌\s*대출|보금자리론/;
